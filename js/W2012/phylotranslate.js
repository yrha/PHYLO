...
(function() {
	...
	(function() {
		...
		g.prototype.dumpMusic = function() {
			...
			var data = "";
			var gs = this.gs;
			var data = "<audio controls autoplay='autoplay' loop='loop' id='game-audio' preload='auto' autobuffer style='display:none'><source src='http://www.cs.mcgill.ca/~jeromew/pub/Valent/Valent%20-%20The%20Buckle.mp3' />Your browser does not support audio element</audio><div id='audioPlayer'><a href='#' class='player' id='playaudio' style='display:none'><div></div></a><a href='#' class='player' id='pause'><div></div></a></div>";
			$("#sandbox").append(data);	
...
*/
	...
	(function() {
		...
		g.prototype.splashLogo = function(doc, win) {
				...
				if(doc.getElementById("playButton") == undefined) {
					...
					if(mcb.cookie.read("PhyloFBLogin") != null || mcb.cookie.read("PhyloLogin") != null) {
						...
						div.innerHTML = "Welcome back! Not "+(mcb.cookie.read("PhyloFBLogin")==null?mcb.cookie.read("PhyloLogin"):mcb.cookie.read("PhyloFBLogin").replace(/\/.*/,""))+"? <a href='#' id='logout'>Log Out</a>";
		...
		g.prototype.endGameMenu = function(value) {
			...
			$("#endGameMenu").css({
				"position" : "absolute",
				"top" : ($("#menu").css("display") == "block")?parseInt($("#menu").css("height"))+parseInt($("#menu").css("top")):0,
		...
		g.prototype.displayGameMenu = function(doc,win) {
			...
			var gMenu = {
				...
				disease : function() {
					...
					var data = "";
					var color = [
							["digestive","../img/digestive.png"],
							["heart", "../img/heart.png"],
							["cancer", "../img/cancer.png"],
							["metabolic", "../img/metablic.png"],
							["blood","../img/blood.png"],
							["sensory","../img/sensory.png"],
							["brain","../img/brain.png"]];
					for(var i=0;i<color.length;i++)
						data+="<a href='#' class='gameOpt'><img src='"+color[i][1]+"</img></a>";
					$("#gameContent").html(data);
					$(".gameOpt").click(function(){
						var url = "../phpdb/phyloDB2.php";
						var score = $(this).html();
		...
		g.prototype.cssScore = function() {
			...
			if(common.checkDevice() == "other") {
				...
				$("#score").css("left",gs.xPx*gs.xMargin+(gs.x-8)*gs.xPx);
			}
		};
		...
		g.prototype.buildCompactTree = function() {
			...
			if(tree.child == 0) {
				...
				y1 = gs.yPx*gs.yMargin + gs.yPx*(tree.node1);
				dy1 = gs.yPx*gs.yMargin + gs.yPx*(tree.node1-.3);
				...
				y1 = gs.yPx*gs.yMargin + gs.yPx*(tree.node2);
				dy1 = gs.yPx*gs.yMargin + gs.yPx*(tree.node2-.3);
		...
		g.prototype.tutorial = function() {
			...
			var show = {
				begin : function() {
					...
					var tutorialtext;
					if (mcb.GET("lang") == "fr" ) tutorialtext = "D&eacute;placez les carr&eacute;s &agrave; gauche et &agrave; droite. Essayez de faire correspondre les couleurs dans une colonne tout en &eacute;vitant les lacunes.";
					else tutorialtext = "<div>Move the squares left and right. Try to match colours vertically while avoiding gaps.</div>";
				...
				score : function() {
					...
					$("#tutorial-score-circle").css({
						...
						"border-style" : "solid",
						"border-color" : "#FFFFFF",
						"top" : parseInt($(".compactTreeNode").css("top").replace(/px/,"")) - gs.xPx*2,//(gs.yMargin-0.5)*gs.xPx,
					...
					$("#tutorial-par-circle").css({
						...
						"left" : parseInt($("#game-clock").css("left").replace(/px/,""))+window.optionScale - gs.xPx*1.5//gs.xMargin*gs.xPx+(gs.x-2)*gs.xPx
					});
					var tutorialtext;
	...
	g.prototype.onSelect = function(doc,win,e) {
			...
			//	$("#multiSelectCircleMove").html("X: "+e.pageX+" - "+(correction+gs.xPx)+"<br>Y: "+e.pageY +" - "+(gs.xPx));
				if(e.pageX - correction >= pivotX && e.pageY >= pivotY) { //down right
					$("#multiSelectSquare").css({
						...
						"width" : e.pageX - correction - pivotX - gs.xPx/3
					});	
				} else if( e.pageY >= pivotY && e.pageX < (pivotX+correction)) { //down left
					$("#multiSelectSquare").css({
						"top" : pivotY,
						"left" : e.pageX - correction,
						"height" : e.pageY - pivotY - gs.xPx/3, 
						"width" : (pivotX+correction) - e.pageX
					});
				} else if( e.pageY < pivotY && e.pageX < (pivotX+correction)) { //top left
					$("#multiSelectSquare").css({
						"top" : e.pageY,
		...
		g.prototype.onSelectCapture = function(correction,doc,win) {
			...
			if(selectTracking) {
				...
				var handler = function(e) {
					if(box.left <= e.pageX-os && e.pageX-os <= box.right
						&& box.top <= e.pageY && e.pageY <= box.bottom) {
