...
(function() {
	...
	(function() {
		...
		g.prototype.dumpMusic = function() {
			//var data = "<object width='0' height='0' data='http://www.cs.mcgill.ca/~jeromew/pub/Valent/Valent%20-%20The%20Buckle.mp3' type='audio/mpeg'><PARAM NAME=autostart VALUE=true><PARAM NAME=hidden VALUE=true></object>";
			var data = "";
			var gs = this.gs;
			//var data = "<audio autoplay='autoplay' loop='loop' id='game-audio' preload='auto' autobuffer style='display:none'><source src='http://www.cs.mcgill.ca/~jeromew/pub/Valent/Valent%20-%20The%20Buckle.mp3' />Your browser does not support audio element</audio><div id='audioPlayer'><a href='#' class='player' id='playaudio' style='display:none'><div><img src='../img/speaker2_Sound.png' class='audioUnMute'/></div></a><a href='#' class='player' id='pause'><div><img src='../img/speaker2_Mute.png' class='audioMute'/></div></a></div>";
			var data = "<audio autoplay='autoplay' loop='loop' id='game-audio' preload='auto' autobuffer style='display:none'><source src='http://www.cs.mcgill.ca/~jeromew/pub/Valent/Valent%20-%20The%20Buckle.mp3' />Your browser does not support audio element</audio><div id='audioPlayer'><a href='#' class='player' id='playaudio' style='display:none'><div><img src='../img/volume-medium.png' class='audioUnMute'/></div></a><a href='#' class='player' id='pause'><div><img src='../img/audio-volume-muted.png' class='audioMute'/></div></a></div>";
			$("#sandbox").append(data);	
			...
			$("#audioPlayer").css({
				position:"absolute",
				top : (gs.y+1.5)*gs.yPx+gs.yPx*gs.yMargin,
			...
			$("#pause div").css({
				position:"relative",
				//background:"url(circleplayer/circle.skin/controls.jpg) 0 -50px no-repeat",
				//background: "url(../img/speaker2_Mute.png) no-repeat",
				width: "50px",
				height:"50px"
			});
			/*
			$("#pause div").hover(function() {
				$(this).css({
					position:"relative",
					background:"url(circleplayer/circle.skin/controls.jpg) -50px -50px no-repeat",
					width: "50px",
					height: "50px"
				});
			}, function() {
				$(this).css({
					position:"relative",
					background:"url(circleplayer/circle.skin/controls.jpg) 0px -50px no-repeat",
					width: "50px",
					height: "50px"
				});
			}); */
			$("#playaudio div").css({
				position:"relative",
				//background:"url(circleplayer/circle.skin/controls.jpg) 0px 0px no-repeat",
				//background: "url(../img/speaker2_Sound.png) no-repeat",
				width: "50px",
				height: "50px"
			});
			/*
			$("#playaudio div").hover(function() {
				$(this).css({
					position:"relative",
					background:"url(circleplayer/circle.skin/controls.jpg) -50px 0px no-repeat",
					width: "50px",
					height: "50px"
				});
			}, function() {
			...
			/*
			data = "<div id='jquery_jplayer_1' class='cp-jplayer'></div>";
			
			data+="<div id='prototype-wrapper'><div id='cp_container_1' class='cp-container'>"+
			...
			*/
			...
			{
				mp3: "http://www.cs.mcgill.ca/~jeromew/pub/Valent/Valent%20-%20The%20Buckle.mp3"
			}, {
...
//			gameIDE.dumpGrid();
		...
		g.prototype.controller = function(x,cache) {
			...
			animate.timer();
			round.start(x);
			window.setTimeout(animate.multiSelect(cache,f.Grid,f.tree,f.gData),0);
			if(common.checkDevice() == "other") 
	...
	(function() {
		...
		g.prototype.algo = function(grid, gs, gData, tree, stage) {				
			...
			if(this.par != undefined) { 
				$("#score").html("Score: "+this.tree[stage].score+"<br><label class='par'>Par: "+this.par[stage]+"</label><br><label class='par'>Stage: "+(stage+1)+"/"+tree.length+"</label><label class='par'>Best Score: "+tree[stage].bestScore+"</label>");
	...
	(function() {
		...
		g.prototype.movePhysicsLoop = function(RIGHT, target,e,gs,c) {
			...
			if(RIGHT)
				console.log("RIGHT @ Time @ > "+elapsed);
			else 
				console.log("LEFT @ Time @ > "+elapsed);
			*/
			return;
		};


		//the part for moving the cells
		/*depreciated took over by movephysicsloop */
		/*
		g.prototype.movePhysics = function(RIGHT,target,e,gs,c) {
			var pos = parseInt(target);
			if(RIGHT) { 		
				$("#"+target).css("zIndex",100-c);
				if((e.pageX-window.optionScale)+(c*gs.xPx)+gs.xPx/2 > ((gs.xPx*gs.xMargin)+(gs.xPx*gs.x))) {
					return;
				}
				if($("#"+(pos+1))) {
					if(((e.pageX-window.optionScale)+(c*gs.xPx)) > parseInt($("#"+(pos+1)).css("left"))-(gs.xPx*.5)) {
						$("#"+target).css("left",((e.pageX-window.optionScale)+c*gs.xPx-(gs.xPx/2)));
						return this.movePhysics(RIGHT,(pos+1),e,gs,(c+1));
					} 
				}
				$("#"+target).css("left",((e.pageX-window.optionScale)+c*gs.xPx-(gs.xPx/2)));
				return;
				...
				if($("#"+(pos-1))) {
					if(parseInt($("#"+(pos-1)).css("left")) > ((e.pageX-window.optionScale)+(c*gs.xPx)-(gs.xPx*1.5))) {
						$("#"+target).css("left",((e.pageX-window.optionScale)+c*gs.xPx-(gs.xPx*0.5)));
						return this.movePhysics(RIGHT,(pos-1),e,gs,(c-1));
					}
				}
				$("#"+target).css("left",((e.pageX-window.optionScale)+c*gs.xPx-(gs.xPx*0.5)));
	...
	(function() {
		...
		g.prototype.splashLogo = function(doc, win) {
				...
				if(doc.getElementById("playButton") == undefined) {
					...
					img.src = "http://phylo.cs.mcgill.ca/img/phylo_logo.png";
					img.id = "splashLogo";
					...
					doc.getElementById("splashPageWrapper").appendChild(div);
					var img = new Image();
					img.src = "http://phylo.cs.mcgill.ca/img/phylo_sitelogo.png";
					img.id = "splashLogo2";
					img.className = "phylo_logo2";
					doc.getElementById("splashPageWrapper").appendChild(img);
		...
		g.prototype.endGameMenu = function(value) {
			...
			$("#endGameMenu").css({
				...
				"width" : parseInt(screen.availWidth)+parseInt($("#sandbox").css("left")),
			...
			if(nice)
				//	$("#endGameMenu-body").html("COMPLETED");
				if (mcb.GET("lang") == "fr" ) context = "F&eacute;lications! Vous avez termin&eacute; l'&eacute;tage finale, et nous avons soumis votre solution. <br>";
				else context = "Congratulations! You've completed the final stage, and we just submitted your alignment for this session. <br>";
			else { //failed
			/* gameID (diff from ingame ID) = this.seqID */
				//console.log(this.roundID);
				if (mcb.GET("lang") == "fr" ) context = "Malheureusement, vous avez manqu&eacute; de temps, donc aucune solution a &eacute;t&eacute; soumise. <br>";
				else context = "Unfortunately, you ran out of time before reaching the final stage, so your alignment was not submitted this session.<br>";
			}
			...
			$("#endGameMenu-button").html("<a href='#' id='newGame' class='endGameMenu-button'>New Game</a><a href='#' id='replayGame' class='endGameMenu-button'>Replay Game</a>");
			...
			var gMenu = {
				random : function() {
					...
					$(".gameOpt").click(function(){
						...
						var score = $(this).html();
						loading.install();
						$.post(url, { "mode" : 1, "diff": score},function(data) {
							$("#gameMenu").hide();
							loading.uninstall();
							data = data.replace("@","");		
							try {
								...
								console.log(data);
								if (mcb.GET("lang") == "fr")
									bug.lightbox("Oops! Le serveur a plant&eacute;!<br> S'il vous pla&icirc;t r&eacute;essayer plus tard.");
								else
									bug.lightbox("Oops! The Server Crashed!<br> Please Try again later");
								return;
							}
							json = json[0].level;
				...
				disease : function() {
					...
					var disease = f.getDiseaseList();
					$("#gameContent").html("");		
					var list = "<select id='game-disease-select'>";
					for(var i=0;i<disease.length;i++)
						list+= "<option value='"+i+"'>"+disease[i][0]+"</option>";
					list += "</select>";
					var data = "<div id='game-disease'>Disease: "+list+"<br><a href='#' id='game-disease-submit'>Submit</a></div>";
					$("#gameContent").html(data);

					if(common.checkDevice() !="other") {
						$("#game-disease-select").css("fontSize","1.5em");
					}
					...
					$("#game-disease-submit").click(function() {
						loading.install();
						var url = "../phpdb/phyloDB2.php";
						var name = $.trim($("#game-disease-select").val());
						var id = disease[name][1][Math.floor(Math.random()*disease[name][1].length)];
						$.post(url, { "mode": 2,"id": id},function(data) {
							loading.uninstall();
							if(data == "") {
		...
		g.prototype.createAngle = function(y1, x1, y2, x2) {
			...
			if(y2>y1) {
				...
				vertical.style.left = x1+"px";
				vertical.style.height = (y1-y2)+"px";
				vertical.style.borderLeft = "solid 2px #FFF";
				horizontal.style.top = y2+"px";
				horizontal.style.left = x1+"px";
				horizontal.style.width = (x2-x1) + "px";
		...
		g.prototype.cssScore = function() {
			var gs = this.gs;
			$("#score").css("top",(gs.y+1.3)*gs.yPx+gs.yPx*gs.yMargin);
			if(common.checkDevice() == "other") {
			...
			if(1024 > window.innerWidth) {
				$("#score").css({
					"font-size" : ".75em",
					"left" : gs.xPx*gs.xMargin+(gs.x-6.5)*gs.xPx
				});
			}
		};
		...
		//	console.log(this.testFontSize());
			...
			$("#compactTree").append("<div class='compactTreeNodeFake' id='ctnf"+this.stage+"'style='top:"+top+"px;left:"+left+"px'></div>" +
						"<div class='compactTreeNode' id='ctn"+this.stage+"'style='top:"+(top-gs.xPx/6)+"px;left:"+(left-gs.yPx/6)+"px;height:"+gs.xPx/3+"px;width:"+gs.yPx/3+"px'></div>" +
						"<div class='compactTreeNodeScore' id='ctn-score-"+this.stage+"'style='top:"+(top-gs.xPx/2)+"px;left:"+(left-gs.yPx/1.7)+"px;height:"+gs.xPx/3+"px;width:"+gs.yPx/1.5+"px;font-size:"+gs.xPx/3.4+"px'>"+score+"</div>");
			var f = this;
			if(common.checkDevice() == "other") {
		...
		g.prototype.tutorial = function() {
			...
			var show = {
				begin : function() {
					...
					$("#tutorialContent").append("<a id='tutorial-continue' href='#'>Continue"+tutorialtext+"</a><div id='tutorial-content-begin'><div class='arrow-left arrow-color'></div><div class='arrow-mid arrow-color'></div><div class='arrow-right arrow-color'></div></div>");
					...
					var tutorialtext;
					if (mcb.GET("lang") == "fr" ) tutorialtext = "<div>D&eacute;placez les carr&eacute;s &agrave; gauche et &agrave; droite. Essayez de faire correspondre les couleurs dans une colonne tout en &eacute;vitant les lacunes.<br>"+(common.checkDevice()=="ipad"?"Utilisez deux doigts":"Double-cliquez")+" pour cr&#233;er une bo&#238;te pour s&#233;lectionner plusieurs blocs &#224; la fois.</div>";
					else tutorialtext = "<div>Move the squares left and right. Try to match colours vertically while avoiding gaps.<br>"+(common.checkDevice()=="ipad"?"Use two fingers":"Double-click")+" to create a selection box for multiple blocks.</div>";
				...
				score : function() {
					...
					var tutorialtext;
					if (mcb.GET("lang") == "fr" ) tutorialtext = "Regardez ici pour voir comment vous faites. Les scores rouges signifie que vous pouvez am&eacute;liorer cette branche!";
					else tutorialtext = "Check here to see how you're doing. Red scores mean that you can improve that branch!";
					...
					if (mcb.GET("lang") == "fr" ) tutorialtext = "Regardez ici pour le score 'par'. C'est le score minimum dont vous avez besoin pour passer &agrave; l'&eacute;tage suivante.<br> Cliquez sur l'&eacute;toile qui saute ici quand vous &ecirc;tes pr&ecirc;t!";
					else tutorialtext = "Look here for the par score. That is the minimum score you need to move to the next stage.<br> Click the jumping star here when you are ready!";
					$("#tutorial-par-content").html(tutorialtext);
					$("#tutorialContent").show();
					$("#tutorialContent-bg").show();
					if (mcb.GET("lang") == "fr" ) tutorialtext = "Je Suis Pr&ecirc;t!";
					else tutorialtext = "I'm Ready!";
